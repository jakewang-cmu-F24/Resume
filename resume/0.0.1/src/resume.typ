// Modified from basic-resume@0.23
...
#let resume(
  ...
  show heading.where(level: 2): it => [
    #pad(top: -8pt, bottom: -10pt, [#smallcaps(it.body)])
  ...
  let contact-item(value, prefix: "", link-type: "", display: "") = {
    if value != "" {
      if link-type != "" and display == "" {
        link(link-type + value)[#(prefix + value)]
      } else if link-type != "" and display != "" {
  ...
  align(personal-info-position)[
    #{
      let items = (
        ...
        contact-item(github2, link-type: "https://"),
...
#let project(
  ...
  generic-one-by-two(
    left: {
      if role == "" {
        [*#name* #if url != "" and dates != "" [ (#link("https://" + url)[Link])]]
      } else {
